...
const palabrasIniciales = ['libro', 'perro', 'tucan', 'jaula', 'lapiz',"lobo","flauta"];
...
export function Game({params}) {
  ...
  const [vidas, setVidas] = useState(params);
  ...
  const cuadrosEntrada = valoresEntrada.map((valor, indice) => (
    <div key={indice} className="flex flex-col-reverse items-center justify-center ">
      <input
        ...
        className='w-32 text-center text-black border-2 border-gray-400'
      />
...
<button onClick={manejarReiniciar} className='p-2 mt-32 text-white bg-slate-500'>Reiniciar</button>
      ...
      {(vidas == 0) && <Link href={"/"} className='p-2 mt-5 text-white bg-blue-700'>Regresar al inicio</Link>}
    </div>
  );
}
